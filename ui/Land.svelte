<script lang="ts">
  ...
  const { at }: { at: string } = $props()

  SS.createStoreContext({ at: at })
...
/> -->
  ...
  >
    {#each Object.entries(S.framesComponents) as [name, { meta, Component, code, inner }] (name)}
      <Frame {name} {code} {Component} {meta} {inner} />
    {/each}
